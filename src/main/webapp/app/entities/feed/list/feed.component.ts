...
import { EntityArrayResponseType, FeedService } from '../service/feed.service';
...
export class FeedComponent implements OnInit {
  ...
  trackId = (item: IFeed): number => this.feedService.getFeedIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.feeds.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(data: IFeed[] | null): IFeed[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.feedService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
