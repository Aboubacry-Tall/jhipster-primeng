...
type RestOf<T extends IArticle | NewArticle> = Omit<T, 'publishedDate'> & {
...
export class ArticleService {
  ...
  protected convertDateFromClient<T extends IArticle | NewArticle | PartialUpdateArticle>(article: T): RestOf<T> {
    return {
      ...article,
      publishedDate: article.publishedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restArticle: RestArticle): IArticle {
    return {
      ...restArticle,
      publishedDate: restArticle.publishedDate ? dayjs(restArticle.publishedDate) : undefined,
    };
  }
